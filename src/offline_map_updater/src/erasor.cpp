...
void ERASOR::set_inputs(const pcl::PointCloud<pcl::PointXYZI> &map_voi,
  ...
  //  ="); ROS_INFO_STREAM(" \033[1;36m"<<debug_total_num +
  //  map_complement.points.size()<<"\033[0m | "<<debug_total_num << " +
  ...
  for (auto const &pt : src.points) {
    if (pt.z < max_h && pt.z > min_h) {
      ...
      if (r <= max_r) {
        ...
        int sector_idx =
            min(static_cast<int>((theta / sector_size)), num_sectors - 1);
...
void ERASOR::compare_vois_and_revert_ground(int frame) {
  ...
  //  std::string filename = "/home/shapelim/debug/" + std::to_string(frame)
  //  +".csv"; ofstream output(filename.data());
    ...
    for (int r = 0; r < num_rings; r++) {
      ...
      if (bin_curr.is_occupied && bin_map.is_occupied) {
        ...
        if (scan_ratio < scan_ratio_threshold) {  // find dynamic!
          ...
          if (map_h_diff >= curr_h_diff) {  // Occupied -> Disappear  <<green>>
            ...
            if (bin_map.max_h > th_bin_max_h) {
              ...
              r_pod_selected[r][theta] = bin_map;
                     ...
                     curr_h_diff) {  // No objects exist -> Appear! <<red>>
            auto polygons = set_polygons(r, theta, 3);
            polygons.header = poly_list.header;
            poly_list.polygons.push_back(polygons);
          ...
          r_pod_selected[r][theta] =
              bin_merged;  // present scene is better I think?
        }
  ...
  // For debugging
  sensor_msgs::PointCloud2 pc2_map_r =
      erasor_utils::cloud2msg(debug_map_rejected);
  sensor_msgs::PointCloud2 pc2_curr_r =
      erasor_utils::cloud2msg(debug_curr_rejected);
...
void ERASOR::compare_vois_and_revert_ground_w_block(int frame) {
  ...
  //  pcl::PointCloud<pcl::PointXYZI> origin_total, ground_total,
  //  dummy_non_ground;
      ...
      if (OCCUPANCY_STATUS == LITTLE_NUM) {
        ...
        if ((bin_map.max_h - bin_map.min_h) > 0.5) {
          ...
          *tmp = r_pod_selected[r][theta].points;
          ...
          erasor_utils::count_stat_dyn(piecewise_ground_, num_ground_stat,
                                       num_ground_dyn);
          ground_viz += piecewise_ground_;
          debug_map_rejected += non_ground_;
...
bool ERASOR::is_dynamic_obj_close(R_POD &r_pod_selected, int r_target,
  ...
  for (int r = std::max(0, r_target - r_range);
       r <= std::min(r_target + r_range, num_rings - 1); r++) {
...
void ERASOR::get_static_estimate(pcl::PointCloud<pcl::PointXYZI> &arranged,
  ...
  sensor_msgs::PointCloud2 pc2_map_complement =
      erasor_utils::cloud2msg(complement);
  pub_arranged.publish(pc2_arranged);
}

double ERASOR::get_max_range() { return max_r; }

geometry_msgs::PolygonStamped ERASOR::set_polygons(int r_idx, int theta_idx,
                                                   int num_split) {
