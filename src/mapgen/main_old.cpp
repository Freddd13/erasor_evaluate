...
void saveGlobalMap() {
  ...
  std::string map_dir = save_path + "/" + sequence + "_" + init_stamp + "_to_" +
                        final_stamp + "_w_interval" + std::to_string(interval) +
                        "_voxel_" + std::to_string(voxelsize) + ".pcd";
...
void callbackData(const node msg) {
  ...
  if ((cnt % viz_interval) == 0) {
    std::cout << std::left << setw(nameWidth) << setfill(separator)
              << "[MAPGEN] " << msg.header.seq << "th frame comes!"
              << std::endl;
  }
  ...
  // Visualization
  if ((cnt % viz_interval) == 0) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloudCurr(
        new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloudMap(
        new pcl::PointCloud<pcl::PointXYZI>());
...
int main(int argc, char **argv) {
  ...
  mapgenerator.setValue(save_path, voxelsize, sequence, init_stamp, final_stamp,
                        interval, is_large_scale);
  mapPublisher =
      nodeHandler.advertise<sensor_msgs::PointCloud2>("/mapgen/map", 100);
  cloudPublisher =
      nodeHandler.advertise<sensor_msgs::PointCloud2>("/mapgen/curr", 100);
  pathPublisher = nodeHandler.advertise<nav_msgs::Path>("/path", 100);

  ros::Subscriber subData = nodeHandler.subscribe<node>(
      "/node/combined/optimized", 3000, callbackData);
  ros::Subscriber subSaveFlag = nodeHandler.subscribe<std_msgs::Float32>(
      "/saveflag", 1000, callbackSaveFlag);
  ros::spin();
